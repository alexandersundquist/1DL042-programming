...
def print_links_from_url(url):
    return print_list(dict_to_list(find_all_links(open_url(url))))

def testfunction():
    testfall = ["http://user.it.uu.se/~joachim/"]
    for test in testfall:
        print(test, "innehåller följande länkar:"), print_links_from_url(test)
